...
int main(int argc, char *argv[]) {
  ...
  do {
    msgq.mtype = 1 ;
    cout << "(Client-1) Enter a message: " ;
    cin >> msgq.mtext ;
    if(msgsnd(msqid1, &msgq, strlen(msgq.mtext)+1, IPC_NOWAIT) < 0) {
      cout << "msgsnd() failed" << endl ;
      exit(1) ;
    }
    /* Message sent */

    if(msgrcv(msqid2, &msgq, MAX_SIZE, 1, 0) < 0) {
      cout << "msgrcv() failed" << endl ;
      exit(1) ;
    }
    cout << "(Client-2) Received message: " << msgq.mtext << endl ;
